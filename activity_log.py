...
def run_activities_module(conn):
    c = conn.cursor()
    ...
    if submit and title:
        ...
        c.execute('''
            INSERT INTO activity_log (date, title, type, location, target_group, description, participants)
            VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    activities = c.execute('''
        SELECT date, title, type, location, target_group, description
        FROM activity_log
        ORDER BY date DESC
    ''').fetchall()
